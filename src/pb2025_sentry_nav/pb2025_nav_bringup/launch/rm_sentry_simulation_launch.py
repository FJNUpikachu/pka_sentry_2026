...
def generate_launch_description():
    ...
    declare_mapping_cmd = DeclareLaunchArgument(
        "mapping",
        default_value="False",
        description="Whether run a MAPPING. If True, it will start mapping",
    )
    ...
    declare_rviz_config_file_cmd = DeclareLaunchArgument(
        "rviz_config_file",
        default_value=os.path.join(bringup_dir, "rviz", "123.rviz"),
    ...
    start_rm_serial_node = Node(
        package='rm_serial_driver',  # 替换为实际的包名
        executable='rm_serial_driver_node',  # 替换为实际的可执行文件名
        name='rm_serial_node',  # 节点名称
        output='screen',
        parameters=[{
            'use_sim_time': use_sim_time,
            # 这里可以添加其他需要的参数
