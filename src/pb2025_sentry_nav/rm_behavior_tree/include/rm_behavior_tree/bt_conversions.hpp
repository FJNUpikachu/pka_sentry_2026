...
inline geometry_msgs::msg::PoseStamped convertFromString(const StringView key)
{
  ...
  auto parts = splitString(key, ';');
  if (parts.size() != 7) {
    throw RuntimeError("invalid input: expected 7 parts separated by semicolons");
  } else {
  geometry_msgs::msg::PoseStamped msg;
  try {
    msg.pose.position.x = std::stod(std::string(parts[0]));
    msg.pose.position.y = std::stod(std::string(parts[1]));
    msg.pose.position.z = std::stod(std::string(parts[2]));
    msg.pose.orientation.x = std::stod(std::string(parts[3]));
    msg.pose.orientation.y = std::stod(std::string(parts[4]));
    msg.pose.orientation.z = std::stod(std::string(parts[5]));
    msg.pose.orientation.w = std::stod(std::string(parts[6]));
  } catch (const std::exception &e) {
    throw RuntimeError("invalid input: " + std::string(e.what()));
  }
  return msg;
  }
}
